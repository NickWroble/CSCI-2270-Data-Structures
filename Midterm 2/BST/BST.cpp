...
void delOddLeafHelper(Node *n){
    if(!n)
        return;
    if(n->left)
        delOddLeafHelper(n->left);
    if(n->right)
        delOddLeafHelper(n->right);
    cout << n->key;
    if(!n->left && !n->right) //node is a leaf
        if(n->key % 2) //node is odd
            if(n->parent->left == n){ //n is to the left of its parent
                n->parent->left = nullptr;
                cout << " (deleting)";
                delete n;
            }
            else if(n->parent->right == n){ //n is to the right of its parent
