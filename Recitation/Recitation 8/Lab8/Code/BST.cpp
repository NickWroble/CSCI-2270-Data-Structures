...
bool isValidBSTHelper(Node* node, int min, int max) {
    ...
    if (node -> key > max)
		return false;
    return isValidBSTHelper(node -> right, node -> key, max) && isValidBSTHelper(node -> left, min, node -> key);			
}

// GOLD TODO
bool BST::isValidBST(){
    if(isValidBSTHelper(root, getMinValueNode(root)->key, getMaxValueNode(root)->key))
        return true;
    return false;
}
